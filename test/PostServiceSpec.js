...
describe('PostServiceFactory', () => {
	...
	it('throws missing loadIdToken function', () => {
		expect(PostServiceFactory.bind(null, {fetch})).to.throw('required "loadIdToken" parameter must be a function');
	});
	describe('post(url, body, headers)', () => {
		it('returns post results', () => {
			const post = PostServiceFactory({fetch, loadIdToken});
			return post('https://test.com/url').then((result) => {
				expect(result).to.eql({test: 'fetch resolve'});
			});
		});
		it('does not call fetch if loadIdToken fails', () => {
